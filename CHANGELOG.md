...
- 修改文件已通过 Swift 前端解析、质量扫描、构建入口检查、Package manifest 和差异检查；Xcode 完整 Simulator 链接仍被外部 Bugly/Metal/SmartCodable 环境阻断。外部流程已创建 `5.6.9` 标签，但该标签不能视为完整验收通过，待修复构建环境后复核。
