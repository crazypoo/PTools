...
class PTPermissionHeader: PTBaseCollectionReusableView {
    ...
    {
        let titleHeight = PTUtils.sizeFor(string: PTPermissionHeader.headerTitle, font: PTAppBaseConfig.share.permissionTitleFont, height: CGFloat(MAXFLOAT), width: kSCREEN_WIDTH - PTAppBaseConfig.share.defaultViewSpace).height + CGFloat.ScaleW(w: 5)
        let infoHeight = PTUtils.sizeFor(string: PTPermissionHeader.headerInfo, font: PTAppBaseConfig.share.permissionSubtitleFont, height: CGFloat(MAXFLOAT), width: kSCREEN_WIDTH - PTAppBaseConfig.share.defaultViewSpace).height + CGFloat.ScaleW(w: 5)
