...
/// iCloud 文件同步管理器
@MainActor
public class PTiCloudFileManager {
    ...
    public func saveFileToICloud(data: Data, fileName: String) {
    ...
    public func readFileFromICloud(fileName: String) -> Data? {
        guard let cloudURL = iCloudDocumentsURL else {
...
extension PTiCloudFileManager {
    ...
    public func backupDatabaseToICloud(dbName: String) {
        guard let cloudURL = iCloudDocumentsURL else {
    ...
    /// 将 iCloud 中的数据库恢复到本地
    /// - Parameter dbName: 数据库文件名，例如 "myDatabase.sqlite"
    public func restoreDatabaseFromICloud(dbName: String) -> Bool {
        guard let cloudURL = iCloudDocumentsURL else {
