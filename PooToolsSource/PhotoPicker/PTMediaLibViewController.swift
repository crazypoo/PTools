...
//            self.returnFrontVC()
            ...
            self.mediaListView.selectImageBlock = { results,isOriginal in
                PTNSLogConsole(">>>>>>>>>>>>>>>>>>>>>>\(results),\(isOriginal)")
            }
            PTNSLogConsole("\(String(describing: self.mediaListView.selectedModel.first!.avEditorOutputItem))")
        }
        return view
    }()
