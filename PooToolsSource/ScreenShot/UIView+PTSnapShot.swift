...
extension UIView: SnapshotKitProtocol {
    @objc public func takeSnapshotOfVisibleContent() -> UIImage? {
        return self.takeSnapshotOfFullContent(for: self.bounds)
    }
    
    @objc public func takeSnapshotOfFullContent() -> UIImage? {
        return self.takeSnapshotOfFullContent(for: self.bounds)
    }

    @objc public func asyncTakeSnapshotOfFullContent(_ completion: @escaping ((UIImage?) -> Void)) {
