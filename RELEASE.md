...
当前仓库的已发布基线为 `5.6.0`，下一候选版本为 `5.6.1`。本轮 5.6.1 ScrollBanner
和 PageControl 治理完成前不创建版本标签；发布目标为 `5.6.1`，`5.6.0` 的既有元数据仍需保持可校验。`Package.swift`
...
5.6.1 的发布还必须满足：Core 源文件契约无漂移、重复入口报告没有未处理的新增组、
迁移文档已更新，并且 Xcode Debug/Release 不是由外部 Pods 或链接器问题阻断。未满足
条件时只保留阻断记录，不创建 `5.6.1` 标签。
