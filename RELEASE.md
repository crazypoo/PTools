...
当前仓库的已发布基线为 `5.6.1`，下一候选版本为 `5.6.2`。本轮 5.6.2 Language
通知链路修复完成前不创建版本标签；发布目标为 `5.6.2`，`5.6.1` 的既有元数据仍需保持可校验。`Package.swift`
...
5.6.2 的发布还必须满足：Language 通知和 String Catalog 回归通过、Core 源文件契约无漂移、重复入口报告没有
未处理的新增组、迁移文档已更新，并且 Xcode Debug/Release 不是由外部 Pods、Swift Package
依赖或链接器问题阻断。未满足条件时只保留阻断记录，不创建 `5.6.2` 标签。
