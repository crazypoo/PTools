...
public class PooSegmentConfig: NSObject {
    ...
    ///加载中图片
    var segDefaultsPlaceHolderImage:UIImage? = UIImage.init()
    ///图片加载选项
...
//    private let lineSqare:CGFloat = 5
    ...
    init(config:PooSegmentConfig,subViewModels:PooSegmentModel,contentW:CGFloat) {
        ...
        if PTBridgeObject.kStringIsEmpty(subViewModels.imageURL ?? "") && !PTBridgeObject.kStringIsEmpty(subViewModels.titles ?? "") && PTBridgeObject.kStringIsEmpty(subViewModels.selectedImageURL ?? "")
        {
        ...
        else if !PTBridgeObject.kStringIsEmpty(subViewModels.imageURL ?? "") && PTBridgeObject.kStringIsEmpty(subViewModels.titles ?? "") && !PTBridgeObject.kStringIsEmpty(subViewModels.selectedImageURL ?? "")
        {
            //TODO:图片地址判断
            ...
            {
                self.label.sd_setImage(with:  URL.init(string: subViewModels.imageURL!), for: .normal, placeholderImage: self.viewConfig.segDefaultsPlaceHolderImage!, options: self.viewConfig.segImageLoadOption!, context: nil)
            }
            else
            {
            ...
            {
                self.label.sd_setImage(with:  URL.init(string: subViewModels.selectedImageURL!), for: .selected, placeholderImage: self.viewConfig.segDefaultsPlaceHolderImage!, options: self.viewConfig.segImageLoadOption!, context: nil)
            }
            else
            {
        ...
        else if !PTBridgeObject.kStringIsEmpty(subViewModels.imageURL ?? "") && !PTBridgeObject.kStringIsEmpty(subViewModels.titles ?? "") && !PTBridgeObject.kStringIsEmpty(subViewModels.selectedImageURL ?? "")
        {
            ...
            if (subViewModels.imageURL! as NSString).isUrlString()
            {
                self.imageBtn.sd_setImage(with:  URL.init(string: subViewModels.imageURL!), for: .normal, placeholderImage: self.viewConfig.segDefaultsPlaceHolderImage!, options: self.viewConfig.segImageLoadOption!, context: nil)
            }
            else
            {
            ...
            if (subViewModels.selectedImageURL! as NSString).isUrlString()
            {
                self.imageBtn.sd_setImage(with:  URL.init(string: subViewModels.selectedImageURL!), for: .selected, placeholderImage: self.viewConfig.segDefaultsPlaceHolderImage!, options: self.viewConfig.segImageLoadOption!, context: nil)
            }
            else
            {
        ...
        case .UnderLine:
            ...
            underLine.snp.makeConstraints { (make) in
                ...
                {
                    PTBridgeObject.viewBorderRadius(underLine, withRadius: lineHight!/2, withWidth: 0, with: .clear)
                }
            }
        case .Dog:
            self.addSubview(underLine)
        ...
        PTBridgeObject.gcd(after: 0.1) {
            ...
            if datas.count > 0
            {
                datas.enumerated().forEach { (index,value) in
                    ...
                    if PTBridgeObject.kStringIsEmpty(value.imageURL ?? "") && !PTBridgeObject.kStringIsEmpty(value.titles ?? "") && PTBridgeObject.kStringIsEmpty(value.selectedImageURL ?? "")
                    {
                        
                    }
                    else if !PTBridgeObject.kStringIsEmpty(value.imageURL ?? "") && PTBridgeObject.kStringIsEmpty(value.titles ?? "") && !PTBridgeObject.kStringIsEmpty(value.selectedImageURL ?? "")
                    {
                        //TODO:图片地址判断
