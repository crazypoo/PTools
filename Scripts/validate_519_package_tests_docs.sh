...
# English: Gate the 5.19–5.22 package, dependency, API, test, and documentation contracts.
# Español: Protege los contratos de paquetes, dependencias, API, pruebas y documentación de 5.19–5.22.
# 中文：统一门禁 5.19–5.22 的包、依赖、API、测试和文档契约。

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$repo_root"

version="$(tr -d '[:space:]' < VERSION)"
[[ "$version" =~ ^5\.(19|20|21|22)\.[0-9]+$ ]] || { printf 'FAIL: package/docs gate requires a 5.19.x, 5.20.x, 5.21.x or 5.22.x VERSION, got %s\n' "$version" >&2; exit 1; }
