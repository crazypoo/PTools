...
struct SettingsManager {
	...
	static func showSettingsActionSheet(on viewController: UIViewController, completionHandler: @escaping (HideOption) -> Void) {
        ...
        alert.addAction(UIAlertAction(title: "settings.hide.title".lt_localized, style: .default, handler: { (_: UIAlertAction) in
            let alert = UIAlertController(title: "settings.hide.sheet.title".lt_localized, message: nil, preferredStyle: .actionSheet)
            alert.addAction(UIAlertAction(title: "settings.hide.sheet.untilMoreIssue".lt_localized, style: .default, handler: { (_: UIAlertAction) in
                completionHandler(.untilMoreIssue)
            }))
            alert.addAction(UIAlertAction(title: "settings.hide.sheet.untilNewType".lt_localized, style: .default, handler: { (_: UIAlertAction) in
                completionHandler(.untilNewIssueType)
            }))
            alert.addAction(UIAlertAction(title: "settings.hide.sheet.untilRestart".lt_localized, style: .default, handler: { (_: UIAlertAction) in
                completionHandler(.always)
            }))
            alert.addAction(UIAlertAction(title: "settings.cancel".lt_localized, style: .cancel, handler: { (_: UIAlertAction) in
